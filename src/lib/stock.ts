...
export function indexNamesByArticle(rows: StockRow[]): Record<string, string> {
  ...
  for (const r of rows) {
    ...
    // Ячейка «Наименование» в таблице смёржена на всю серию — один и тот же
    // текст для всех моделей группы, без артикула внутри. Берём его как
    // есть, ничего не дописываем: иначе это уже не «точно как в файле».
    const ru = r.name.split(/\s*\/\s*/)[0].replace(/\s+/g, ' ').trim();
    if (ru) map[key] = ru;
  }
  return map;
}
