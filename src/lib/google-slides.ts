...
export async function generateSlidesKP(data: {
  ...
  aggregated.forEach(item => {
    ...
    const img = item.slidesImage && item.slidesImage.startsWith('http') ? item.slidesImage : item.image;
    
    // Only group with PREVIOUS if it matches exactly
    let group = groups.length > 0 ? groups[groups.length - 1] : null;
    if (group && group.category === cat && group.series === ser && group.image === img) {
      group.models.push({ 
  ...
  if (tablesData.length > 1) {
    ...
    for (let i = 1; i < tablesData.length; i++) {
        ...
        duplicateReqs.push({
            duplicateObject: {
                objectId: allSlideIds[0],
                objectIds: { [allSlideIds[0]]: newId }
            }
        });
  ...
  // 4. Fill placeholders (replaces on ALL slides at once)
  const placeholders = [
  ...
  // 5. Pre-upload local images (Drive URLs are used directly)
  const fileIdsToDelete: string[] = [];
  const imageMap = new Map<string, string>();
  if (opts.showImages) {
    const uniqueImages = [...new Set(groups.map(g => g.image))];
    console.log(`SpeedUp: Parallel upload of ${uniqueImages.length} images...`);
      ...
      try {
        if (imgPath.startsWith('http')) {
          ...
          if (fs.existsSync(fullPath)) {
            ...
            // Small delay to allow Drive to index the permission
            await new Promise(r => setTimeout(r, 500));
            const info = await drive.files.get({ fileId, fields: 'thumbnailLink' });
            const finalUrl = info.data.thumbnailLink ? info.data.thumbnailLink.replace('=s220', '=s1000') : `https://drive.google.com/uc?id=${fileId}`;
  ...
  // Pre-upload itogo logo
  let itogoLogoUrl = '';
  const itogoLogoPath = path.join(process.cwd(), 'logo near itogo.png');
  if (fs.existsSync(itogoLogoPath)) {
    try {
      const logoUpload = await drive.files.create({
        requestBody: { name: `kp_logo_itogo_${Date.now()}`, mimeType: 'image/png' },
        media: { body: fs.createReadStream(itogoLogoPath) },
        fields: 'id'
      });
      const logoFileId = logoUpload.data.id!;
      fileIdsToDelete.push(logoFileId);
      await drive.permissions.create({ fileId: logoFileId, requestBody: { role: 'reader', type: 'anyone' } });
      await new Promise(r => setTimeout(r, 1000));
      const logoInfo = await drive.files.get({ fileId: logoFileId, fields: 'thumbnailLink' });
      itogoLogoUrl = logoInfo.data.thumbnailLink ? logoInfo.data.thumbnailLink.replace('=s220', '=s1000') : `https://drive.google.com/uc?id=${logoFileId}`;
    } catch (err) { console.error('Itogo logo upload failed', err); }
  }
  ...
  const sumLabel = opts.currency === 'sum' ? 'Сумма (сум)' : 'Сумма (у.е.)';
  ...
  for (let t = 0; t < tablesData.length; t++) {
      ...
      if (isLastTable) {
          ...
          // Place itogo logo as overlay image on merged cells
          if (itogoLogoUrl) {
            const mergedWidth = columnWidths.slice(0, totIdxL).reduce((a, b) => a + b, 0);
            const itogoRowY = TABLE_START_Y + HEADER_FOOTER_H + (tData.rows * PRODUCT_ROW_H) + (opts.paymentType === 'transfer' ? HEADER_FOOTER_H : 0);
            const logoH = 350000;
            const logoW = 2800000;
            const logoX = TABLE_X + (mergedWidth - logoW) / 2;
            const logoY = itogoRowY + (HEADER_FOOTER_H - logoH) / 2;
            imageRequests.push({ createImage: { url: itogoLogoUrl, elementProperties: { pageObjectId: sId, size: { width: { magnitude: logoW, unit: 'EMU' }, height: { magnitude: logoH, unit: 'EMU' } }, transform: { scaleX: 1, scaleY: 1, translateX: logoX, translateY: logoY, unit: 'EMU' } } } });
          }
      }
  ...
  } catch (e) { console.error('Audit failure', e); }
