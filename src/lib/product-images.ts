/**
 * Realistic product image resolver for UMBT catalog.
 * Guarantees that every equipment model, series, and category maps to an authentic,
 * high-definition official Midea equipment photo.
 */
...
/**
 * Normalizes Google Drive links so they can be safely displayed or opened by users.
 */
export function formatDriveLink(url?: string): string | null {
  if (!url) return null;
